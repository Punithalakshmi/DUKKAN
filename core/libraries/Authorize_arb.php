...
class Authorize_arb
{
	...
	public function startData( $type )
	{
		...
		$get_authorize_payment_credentials = $this->CI->db->query("select api_username,api_password,url from payment_info where name='authorize'")->row_array();

		$this->api_login_id = $get_authorize_payment_credentials['api_username'];			// API Login ID

        $this->api_transaction_key = $get_authorize_payment_credentials['api_password'];	// API Transation Key
    
        $this->arb_api_url = $get_authorize_payment_credentials['url'];			// Where we postin' to?
